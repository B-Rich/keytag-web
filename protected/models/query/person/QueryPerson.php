    ...
    public function search($pageSize=20){
       ...
       // $this->applySecurity($with);
        ...
        $sort->attributes = array(
            'Household'=>array(
                'asc'=>'ISNULL(HouseholdRelation.Name), HouseholdRelation.Name ASC',
    ...
    private function applySecurity(&$with){
        if (!Yii::app()->user->checkAccess(UserRoles::SuperAdmin)) {
            if (Yii::app()->user->checkAccess(UserRoles::LocalAdmin)) {//local admin
                $with['HouseholdRelation'] = array(
                    'join' => 'INNER JOIN locations l ON l.ID = HouseholdRelation.Location',
                    'condition' => Locations::model()->getLocationClause($userId),
                );
            } else {// School representative
                $with['SchoolRelation'] = array(
                    'join' => 'INNER JOIN locations l ON l.ID = SchoolRelation.Location',
                    'condition' => Locations::model()->getLocationClause($userId),
                );
            }
        }
